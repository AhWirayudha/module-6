...
describe("ProfilePage", () => {
  ...
  it("shows validation error if birth date is in the future", async () => {
    ...
    // Try to find birth date input robustly
    let birthDateInput;
    try {
      birthDateInput = screen.getByLabelText(/Birth Date/i);
    } catch {
      // Fallback: try by role and type
      const allInputs = screen.getAllByRole("textbox");
      birthDateInput = allInputs.find(
      ...
      if (!birthDateInput) {
        ...
        throw new Error("Birth Date input not found");
      }
    }
